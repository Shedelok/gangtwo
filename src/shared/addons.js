...
export const NEGATIVE_ADDON_TREE = [
    { children: [
...
export const POSITIVE_ADDON_TREE = [
 ...
 */
function pruneNode(node, allowed) {
    if ('addonId' in node) {
 ...
 */
export function pickAddonsFromTree(tree, pool, count) {
    const selected = [];
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (id === null)
            break; // no more addons available
        selected.push(id);
 ...
 */
export function countAvailableInTree(tree, pool) {
